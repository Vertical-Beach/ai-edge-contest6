...
int main(){
    ...
    unsigned int* DMEM_BASE = (unsigned int*) mmap(NULL, 0x8000, PROT_READ|PROT_WRITE, MAP_SHARED, uio0_fd, 0);
    int uio1_fd = open("/dev/uio5", O_RDWR | O_SYNC);
    unsigned int* IMEM_BASE = (unsigned int*) mmap(NULL, 0x8000, PROT_READ|PROT_WRITE, MAP_SHARED, uio1_fd, 0);
    if (uio0_fd < 0 || uio1_fd  < 0) {
        printf("Device Open Error");
        exit(-1);
    }
    //set RISC-V Instruction
    IMEM_BASE[0] = 0xA0008437; //  0: lui s0,0xA0008000
	IMEM_BASE[1] = 0x00040413; //  4: mv  s0,s0
	IMEM_BASE[2] = 0x00042607; //  8: flw  fa2,0(s0) # 0xA0080000
	IMEM_BASE[3] = 0x00442687; //  C: flw  fa3,4(s0)
	IMEM_BASE[4] = 0x00c68753; // 10: fadd fa4,fa2,fa3
	IMEM_BASE[5] = 0x00e42427; // 14: fsw  fa4,8(s0) # 0xA0080000
