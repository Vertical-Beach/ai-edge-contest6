...
from tools.torchprofiler import Profiler
...
class CenterPoint(MVXTwoStageDetector):
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        # calculate FLOPs
        print("Calculating FLOPs of CenterPoint' backbone")
        model = self.pts_backbone
        device = x.device
        profiler = Profiler(model, device="cpu")
        profiler.run_with_input(x.clone().detach().to(torch.device('cpu')))
        ...
        if self.with_pts_neck:
            ...
            # calculate FLOPs
            print("Calculating FLOPs of CenterPoint' neck")
            model = self.pts_neck
            device = x[0].device
            profiler = Profiler(model, device="cpu")
            profiler.run_with_input(tuple(xi.clone().detach().to(torch.device('cpu')) for xi in x))
            profiler.print_summary()
            model.to(device)
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        ...
        # calculate FLOPs
        print("Calculating FLOPs of CenterPoint' head")
        model = self.pts_bbox_head
        device = x[0].device
        profiler = Profiler(model, device="cpu")
        profiler.run_with_input([xi.clone().detach().to(torch.device('cpu')) for xi in x])
        profiler.print_summary()
        model.to(device)
        import sys;sys.exit()
