...
class MVXTwoStageDetector_quant(Base3DDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(MVXTwoStageDetector_quant, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        super(MVXTwoStageDetector_quant, self).init_weights(pretrained)
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        features = self.pts_voxel_encoder.preprocess(voxels, num_points, coors)
        voxel_features = self.pts_voxel_encoder(voxels, features, coors, img_feats, img_metas)
    ...
    def extract_pts_feat_quant(self, voxels, features, coors, img_feats):
        """Extract features of points."""
        if not self.with_pts_bbox:
            return None
        voxel_features = self.pts_voxel_encoder(voxels, features, coors, img_feats)
    ...
    def extract_feat_quant(self, features, voxels, coors, img):
        """Extract features from images and points."""
        img_feats = None
        pts_feats = self.extract_pts_feat_quant(voxels, features, coors, img_feats)
        return (img_feats, pts_feats)

    def preprocess(self, pts):
        voxels, num_points, coors = self.voxelize(pts)
        features = self.pts_voxel_encoder.preprocess(voxels, num_points, coors)
        return features, voxels, coors
    ...
    def postprocess_api(self, outs, img_metas, rescale=False):
        # recover the data structure of outs (after DequantStub by nndct) to the same format as float for decoding
        if os.environ["W_QUANT"]=='1':
        ...
        bbox_final = [dict() for i in range(len(img_metas))]
        for result_dict, pts_bbox in zip(bbox_final, bbox_results):
            result_dict['pts_bbox'] = pts_bbox
        return bbox_final

        
    def simple_test(self, features, voxels, coors, img=None):
        """Test function without augmentaiton."""
        img_feats, pts_feats = self.extract_feat_quant(features, voxels, coors, img=img)

        outs = None
        if pts_feats and self.with_pts_bbox:
            outs = self.simple_test_pts(pts_feats)
        return outs


    def forward_test(self, features, voxels, coors):
        """
        Args:
        ...
        """
        return self.simple_test(features, voxels, coors, None)


    @auto_fp16(apply_to=('img', 'points'))
    def preprocess_api(self, points, img_metas, img=None):
        """
        Args:
            points (list[torch.Tensor]): the outer list indicates test-time
                augmentations and inner torch.Tensor should have a shape NxC,
        ...
        assert num_augs == 1, 'Quantization with augs is not supported'
        img = [img] if img is None else img
        # Somehow, decorator for preprocess_api doesn't work, then points[0] is still DataContainer instead of tensor
        # features, voxels, coors = self.preprocess(points[0])
        features, voxels, coors = self.preprocess(points[0].data[0])
        return features, voxels, coors


    @auto_fp16(apply_to=('img', 'points'))
    def forward(self, features, voxels, coors):
        ...
        augmentations.
        """
        return self.forward_test(features, voxels, coors)
