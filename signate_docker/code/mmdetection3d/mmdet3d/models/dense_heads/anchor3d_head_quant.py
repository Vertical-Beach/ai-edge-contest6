...
class Anchor3DHead_quant(nn.Module, AnchorTrainMixin):
    ...
    def __init__(self,
        ...
        if os.environ["W_QUANT"]=='1':
            self.dequant_list = nn.ModuleList([DeQuantStub() for _ in range(3)])
    ...
    def forward_single(self, x):
        ...
        if os.environ["W_QUANT"]=='1':
            cls_score = self.dequant_list[0](cls_score)
            bbox_pred = self.dequant_list[1](bbox_pred)
            if self.use_direction_classifier:
                dir_cls_preds = self.dequant_list[2](dir_cls_preds)
