...
class SeparateHead_quant(nn.Module):
    ...
    def __init__(self,
                 ...
                 bias='auto',
                 **kwargs):
        super(SeparateHead_quant, self).__init__()
    ...
    def format_separate(self, outs):
        """Reorganize the flatten outputs of a quantized task head to dict format"""
        outs_dict = dict()
        for head, out in zip(self.heads, outs):
            outs_dict.update({head: out})
        return outs_dict
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            ret_dict.append(self.__getattr__(head)(x))

        return ret_dict

@HEADS.register_module()
class CenterHead_quant(nn.Module):
    ...
    def __init__(self,
                 ...
                 bias='auto',
                 norm_bbox=True):
        super(CenterHead_quant, self).__init__()
    ...
    def format_flatten(self, outs):
        """Reorganize the flatten outputs of quantized model to list[dict] format"""
        # len(outs) = len(self.task_heads) * len(self.task_heads[i].heads)
        format_outs = []
        task_len = len(outs) // len(self.task_heads)
        for i, task in enumerate(self.task_heads):
            format_outs.append([task.format_separate(outs[i*task_len:(i+1)*task_len])])
        return tuple(format_outs)
