...
from .utils_qat import VFELayer_deploy_qat, get_paddings_indicator
...
class HardVFE_deploy_qat(nn.Module):
    ...
    def __init__(self,
        ...
        super(HardVFE_deploy_qat, self).__init__()
...
@VOXEL_ENCODERS.register_module()
class HardVFE_deploy_trans_input_qat(nn.Module):
    """Voxel feature encoder used in DV-SECOND.
    ...
    def __init__(self,
        ...
        super(HardVFE_deploy_trans_input_qat, self).__init__()
        assert len(feat_channels) > 0
        if with_cluster_center:
            in_channels += 3
        if with_voxel_center:
