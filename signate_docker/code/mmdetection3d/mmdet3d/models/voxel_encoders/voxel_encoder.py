...
from .utils import VFELayer, VFELayer_deploy, get_paddings_indicator
...
@VOXEL_ENCODERS.register_module()
class HardVFE_deploy(nn.Module):
    """Voxel feature encoder used in DV-SECOND.
    ...
    def __init__(self,
                 ...
                 fusion_layer=None,
                 return_point_feats=False):
        super(HardVFE_deploy, self).__init__()
        assert len(feat_channels) > 0
        if with_cluster_center:
            in_channels += 3
        if with_voxel_center:
    ...
    def forward(self,
        ...
        if os.environ["W_QUANT"]=='1':
            voxel_feats = self.quant(voxel_feats)
...
@VOXEL_ENCODERS.register_module()
class HardVFE_deploy_trans_input(nn.Module):
    """Voxel feature encoder used in DV-SECOND.
    ...
    def __init__(self,
        ...
        super(HardVFE_deploy_trans_input, self).__init__()
        assert len(feat_channels) > 0
        if with_cluster_center:
            in_channels += 3
        if with_voxel_center:
        ...
        # Transform input to be in range [-1, 1]
        # x,y:-50~+50; z:-5~+3; t:0~0.5; painted_one_hot_encoding:{0,1}
        # lbounds = [-50, -50, -5, 0]  + [0] * 11
        # ubounds = [50, 50, 3, 0.5] + [1] * 11
    ...
    def forward(self,
        ...
        # Transform input to be in range [-1, 1]
        # debug
        # mins = [features[:,:,i].min().cpu().numpy().reshape(1)[0] for i in range(features.shape[2])]
        # maxs = [features[:,:,i].max().cpu().numpy().reshape(1)[0] for i in range(features.shape[2])]
        # print(mins)
        # print(maxs)
