...
@PIPELINES.register_module()
class ScalePointsChannels(object):
    """Balance the input channels of point clouds for better quantization & QAT & deployment
    """
    def __init__(self, scales=[1, 1, 1, 1]):
        self.scales = np.array(scales, dtype=np.float32).reshape(1, len(scales))

    def __call__(self, results):
        assert results['points'].shape[-1] == self.scales.shape[-1]
        results['points'] *= self.scales
        return results
