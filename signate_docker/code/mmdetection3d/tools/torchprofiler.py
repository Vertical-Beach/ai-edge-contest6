        ...
        def register_hook(module):
            ...
            def hook(module, input, output):
                ...
                if isinstance(input[0], (list, tuple)):
                    self._summary[m_key]['input_shape'] = [get_data_shape(it) for it in input[0]]
                elif isinstance(input[0], (dict)):
                    print('Warning: dict input may have potential mismatch problem')
                    self._summary[m_key]['input_shape'] = [get_data_shape(it) for it in input[0].values()]
                else:
                    self._summary[m_key]['input_shape'] = get_data_shape(input[0])

                if isinstance(output, (list, tuple)):
                    self._summary[m_key]['output_shape'] = [list(o.size()) for o in output]
                elif isinstance(output, (dict)):
                    print('Warning: dict input may have potential mismatch problem')
                    self._summary[m_key]['output_shape'] = [list(o.size()) for o in output.values()]
                else:
                    print(type(output))
...
def get_data_shape(data):
    if isinstance(data, np.ndarray):
        ...
        return [get_data_shape(e) for e in data]
    else:
